...
import { buildConfiguration, internals, parseEnv, serializeEnv } from "../src/cli.mjs";
import { assertAllowedUrl, isBlockedHost, isLoopbackHost } from "../src/policy.mjs";
...
test("blocks Superset hosts and every subdomain", () => {
  ...
  assert.throws(() => assertAllowedUrl("https://api.superset.sh/v1"), /blocked Superset-operated host/);
  assert.throws(() => assertAllowedUrl("https://api.superset.sh/v1", { blockedHosts: [] }), /blocked Superset-operated host/);
});

test("recognizes only loopback names and addresses", () => {
  for (const host of ["localhost", "127.0.0.1", "127.99.2.3", "::1"]) assert.equal(isLoopbackHost(host), true);
...
test("default-deny permits loopback and explicit infrastructure", () => {
  assert.doesNotThrow(() => assertAllowedUrl("http://127.0.0.1:3000", { networkMode: "default-deny" }));
  assert.doesNotThrow(() => assertAllowedUrl("postgresql://db.internal/app", {
    networkMode: "default-deny",
    allowHosts: ["db.internal"],
  }));
  assert.throws(() => assertAllowedUrl("https://example.com", { networkMode: "default-deny" }), /non-allowlisted/);
});

test("local initialization emits private, local-only service URLs", () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "superset-local-test-"));
  const configFile = path.join(directory, "superset-local.json");
  const built = buildConfiguration({ profile: "local", "allow-host": [] }, configFile);
...
test("local initialization applies custom loopback ports", () => {
  ...
  assert.throws(() => buildConfiguration({ profile: "local", "allow-host": [], "postgres-port": "70000" }), /must be a port/);
});
...
test("external initialization allowlists supplied infrastructure and rejects Superset", () => {
  ...
  assert.deepEqual(built.config.policy.allowHosts, ["cache.corp.test", "db.corp.test", "electric.corp.test"]);
  assert.throws(() => buildConfiguration({ ...base, "electric-url": "https://relay.superset.sh" }), /blocked/);
...
test("doctor validation rejects a tampered non-loopback app endpoint", () => {
  const built = buildConfiguration({ profile: "local", "allow-host": [] }, "/tmp/superset-local.json");
  built.config.endpoints.api = "https://api.corp.test";
  built.config.policy.allowHosts.push("api.corp.test");
  assert.match(internals.validateLoaded(built).errors.join(" "), /api endpoint must use a loopback host/);
});
