...
| §164.312(a)(2)(iii), automatic logoff | Implemented | `SUPERSET_HIPAA_MODE=true` uses a 15-minute renewable session window and disables the five-minute cookie session cache. | Confirm the timeout is appropriate in the risk analysis and separately enforce endpoint/desktop screen locking. Test timeout behavior in each released client. |
