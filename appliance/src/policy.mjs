...
export function assertAllowedUrl(value, options = {}) {
  ...
  const mandatoryBlockedHosts = [...new Set([...DEFAULT_BLOCKED_HOSTS, ...blockedHosts])];
  if (isBlockedHost(host, mandatoryBlockedHosts)) {
    throw new Error(`${label} points at blocked Superset-operated host ${host}`);
  }

  if (networkMode === "default-deny") {
    const normalizedAllowHosts = allowHosts.map(normalizeHost);
    const allowed = isLoopbackHost(host) || normalizedAllowHosts.some((allowedHost) => (
      host === allowedHost || host.endsWith(`.${allowedHost}`)
    ));
    if (!allowed) throw new Error(`${label} points at non-allowlisted host ${host}`);
  }
...
export function createEndpointPolicy(config) {
  ...
  return {
    ...
    async fetch(value, init) {
      const target = typeof Request !== "undefined" && value instanceof Request ? value.url : String(value);
