...
const CONTRACT_FILE = path.join(PACKAGE_ROOT, "integration", "runtime-contract.json");
...
function parseArgs(argv) {
  ...
  const booleanOptions = new Set(["force", "json", "non-interactive", "strict", "help"]);
...
function projectName(directory) {
  const base = path.basename(directory).toLowerCase().replace(/[^a-z0-9_-]+/g, "-").replace(/^-|-$/g, "");
  const hash = crypto.createHash("sha256").update(directory).digest("hex").slice(0, 8);
...
async function promptForMissing(options) {
  ...
  try {
    options.profile ||= (await rl.question("Backing services [local/external] (local): ")).trim() || "local";
    if (options.profile === "external") {
      options["database-url"] ||= (await rl.question("Postgres URL: ")).trim();
      options["database-url-unpooled"] ||= (await rl.question("Unpooled Postgres URL (same): ")).trim() || options["database-url"];
      options["redis-url"] ||= (await rl.question("Redis URL: ")).trim();
      options["electric-url"] ||= (await rl.question("Electric URL: ")).trim();
    }
    options["api-url"] ||= (await rl.question("Local API URL (http://127.0.0.1:3001): ")).trim() || "http://127.0.0.1:3001";
    options["web-url"] ||= (await rl.question("Local web URL (http://127.0.0.1:3000): ")).trim() || "http://127.0.0.1:3000";
    return options;
  } finally {
    rl.close();
  }
...
export function buildConfiguration(options, configFile = configPathFrom(options)) {
  const profile = options.profile ?? "local";
  if (!new Set(["local", "external"]).has(profile)) throw new Error("--profile must be local or external");
  ...
  if (local) {
    ...
    databaseUrlUnpooled = externalValue(options, "database-url-unpooled", "DATABASE_URL_UNPOOLED") || databaseUrl;
    ...
    if (missing.length) throw new Error(`external profile requires ${missing.join(", ")}`);
  ...
  for (const [label, value] of [
    ["API URL", apiUrl], ["web URL", webUrl], ["database URL", databaseUrl],
    ["unpooled database URL", databaseUrlUnpooled], ["Redis URL", redisUrl], ["Electric URL", electricUrl],
  ]) {
    assertAllowedUrl(value, { label, blockedHosts: DEFAULT_BLOCKED_HOSTS, networkMode, allowHosts: [...allowHosts] });
  }
  for (const [label, value] of [["API URL", apiUrl], ["web URL", webUrl]]) {
    if (!isLoopbackHost(hostFromUrl(value, label))) throw new Error(`${label} must use a loopback host`);
  }
  ...
  const config = {
    ...
    profile,
    composeProject: projectName(path.dirname(configFile)),
    ...
    localPorts: local ? { postgres: postgresPort, redis: redisPort, electric: electricPort, neonProxy: neonProxyPort } : null,
    services: {
  ...
  const environment = {
    ...
    NEXT_PUBLIC_ELECTRIC_URL: electricUrl,
...
function loadConfiguration(options) {
  ...
  if (!fs.existsSync(configFile)) throw new Error(`${configFile} does not exist; run superset-local init first`);
  const config = JSON.parse(fs.readFileSync(configFile, "utf8"));
  if (config.schemaVersion !== CONFIG_VERSION) throw new Error(`unsupported config schema ${config.schemaVersion}`);
  const runtimeEnvFile = path.resolve(path.dirname(configFile), config.runtimeEnvFile);
  if (!fs.existsSync(runtimeEnvFile)) throw new Error(`${runtimeEnvFile} is missing; rerun init with --force`);
  return { config, configFile, runtimeEnvFile, environment: parseEnv(fs.readFileSync(runtimeEnvFile, "utf8")) };
}
...
function validateLoaded({ config, environment }) {
  ...
  if (!new Set(["local", "external"]).has(config.profile)) errors.push(`unsupported profile ${config.profile}`);
  if (!new Set(["default-deny", "deny-superset"]).has(config.policy?.networkMode)) {
    errors.push(`unsupported network mode ${config.policy?.networkMode}`);
  }
  for (const [label, value] of Object.entries(config.endpoints ?? {})) {
    try { assertAllowedUrl(value, { ...policyOptions, label: `${label} endpoint` }); } catch (error) { errors.push(error.message); }
  }
  for (const label of ["api", "web"]) {
    try {
      if (!isLoopbackHost(hostFromUrl(config.endpoints?.[label], `${label} endpoint`))) {
  ...
  for (const [key, value] of Object.entries(environment)) {
    ...
    if (key === "SUP_LOCAL_ELECTRIC_DATABASE_URL" && config.profile === "local") {
      try {
        if (hostFromUrl(value, key) !== "postgres") errors.push(`${key} must point at the private Compose postgres service`);
      } catch (error) {
        errors.push(error.message);
      }
      continue;
    }
    try { assertAllowedUrl(value, { ...policyOptions, label: key }); } catch (error) { errors.push(error.message); }
  }
  for (const key of [
    "SUPERSET_CLOUD_DISABLED", "SUPERSET_RELAY_DISABLED", "SUPERSET_TELEMETRY_DISABLED", "SUPERSET_AUTO_UPDATE_DISABLED",
  ]) {
    if (environment[key] !== "true") errors.push(`${key} must be true`);
  }
  if (config.integration?.status !== "verified") {
    warnings.push("upstream application gates are not marked verified; generated flags alone cannot block hard-coded network calls");
  }
  return { errors: [...new Set(errors)], warnings: [...new Set(warnings)] };
}

function composeArgs(loaded, action) {
  const args = ["compose", "-f", COMPOSE_FILE, "--env-file", loaded.runtimeEnvFile, "-p", loaded.config.composeProject];
...
function runDocker(loaded, action) {
  if (loaded.config.profile !== "local") {
    console.log("External profile selected; there are no locally managed backing services.");
    return 0;
  }
  const result = spawnSync("docker", composeArgs(loaded, action), { stdio: "inherit" });
...
async function initCommand(options) {
  ...
  if (!options.force) {
    for (const file of [built.configFile, built.runtimeEnvFile]) {
      if (fs.existsSync(file)) throw new Error(`${file} already exists; use --force to replace it`);
    }
  }
  writePrivateFile(built.runtimeEnvFile, serializeEnv(built.environment), options.force);
  writePrivateFile(built.configFile, `${JSON.stringify(built.config, null, 2)}\n`, options.force);
...
function doctorCommand(options) {
  ...
  try {
    ...
    if (contract.status !== "verified") result.warnings.push(`runtime contract status is ${contract.status}`);
  } catch (error) {
    result.errors.push(`cannot read runtime contract: ${error.message}`);
  }
  if (loaded.config.profile === "local") {
    const docker = spawnSync("docker", ["compose", "version"], { encoding: "utf8" });
    if (docker.error || docker.status !== 0) result.errors.push("Docker Compose is required for the local profile");
  }
  const payload = { ok: result.errors.length === 0 && (!options.strict || result.warnings.length === 0), ...result };
  if (options.json) console.log(JSON.stringify(payload, null, 2));
  else {
    for (const error of result.errors) console.log(`ERROR ${error}`);
    for (const warning of result.warnings) console.log(`WARN  ${warning}`);
    if (!result.errors.length) console.log("OK    configuration contains no Superset-operated endpoints");
  }
...
function runCommand(options) {
  ...
  const command = separator >= 0 ? options._.slice(separator + 1) : options._.slice(1);
  if (!command.length) throw new Error("run needs a command after --");
  const result = spawnSync(command[0], command.slice(1), {
    stdio: "inherit",
    env: { ...process.env, ...loaded.environment, SUPERSET_LOCAL_CONFIG: loaded.configFile },
  });
  if (result.error) throw new Error(`could not run ${command[0]}: ${result.error.message}`);
...
export async function main(argv) {
  ...
  if (command === "up") {
    process.exitCode = runDocker(loadConfiguration(options), ["up", "-d", "--wait"]);
    return;
  }
  if (command === "down") {
...
export const internals = { parseArgs, validateLoaded, loadConfiguration, COMPOSE_FILE };
